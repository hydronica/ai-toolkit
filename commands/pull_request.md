Generate a pull request for the current Git branch. Use the **# PR Template** section at the bottom as the PR body (fill checkboxes and **### Summary** from context).

## Steps

1. **Collect context** with `pr_sum.sh` (summarizes the current branch against the integration base, **checks gh auth**, and **verifies branch push status**). Run it from the **repository you are opening the PR for** (the project worktree, usually the workspace root):
...
2. **Decide with the user before opening a PR**
   ...
   - Fill **### Summary** from commit messages and changed paths; keep checklist items honest.
...
3. **Create the PR** only when step 2 needs no further user decision (or the user explicitly said to continue despite unpushed commits or similar).
   ...
   - Write the completed **# PR Template** block (from this file) to a temporary file and run:
