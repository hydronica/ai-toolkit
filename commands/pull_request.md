Generate a pull request for the current Git branch. The PR body comes from the **PR template** section of `pr_sum.sh` (repo/org template or the built-in default).

## Steps

1. **Collect context** with `pr_sum.sh` (summarizes the current branch against the integration base, **checks gh auth**, **verifies branch push status**, **detects PR templates**, and **prints the template body**). Run it from the **repository you are opening the PR for** (the project worktree, usually the workspace root):
...
2. **Decide with the user before opening a PR**
   ...
   - Fill the template’s summary section from commit messages and changed paths; keep checklist items honest.
...
3. **Create the PR** only when step 2 needs no further user decision (or the user explicitly said to continue despite unpushed commits or similar).
   ...
   - **PR body:** copy the filled template from the `PR template` section of `pr_sum.sh` output into a temporary file, then run:
     `gh pr create --title "<concise title, at most five words>" --body-file <that-file>`
   - When `pr_sum.sh` reports a local template path, you may instead use `--template <path>` with that file after filling it in place.
